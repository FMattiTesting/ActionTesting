...
for j, n_Vec in enumerate(n_Vec_list):
    ...
    error[0, j] = np.sum(np.abs(estimate - baseline)) / np.sum(np.abs(baseline))
    estimate = chebyshev_nystrom(A_st, t, m, n_Vec // 2, n_Vec // 2, kernel)
    error[1, j] = np.sum(np.abs(estimate - baseline)) / np.sum(np.abs(baseline))
    estimate = chebyshev_nystrom(A_st, t, m, 0, n_Vec, kernel)
    error[2, j] = np.sum(np.abs(estimate - baseline)) / np.sum(np.abs(baseline))
...
plt.ylabel(r"$L^1$ relative error")
plt.xlabel(r"estimator size $n_{\mathbf{\Psi}} + n_{\mathbf{\Omega}}$")
plt.legend()
plt.xscale("log")
plt.yscale("log")
plt.savefig("paper/plots/convergence_8_sigma005.pgf", bbox_inches="tight")